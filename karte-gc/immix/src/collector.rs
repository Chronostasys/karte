...
/// 全局GC计数器，用于追踪第几次GC
pub static GC_CYCLE_COUNT: AtomicU64 = AtomicU64::new(0);

/// 当前线程的GC cycle编号
thread_local! {
    static CURRENT_GC_CYCLE: std::cell::Cell<u64> = const { std::cell::Cell::new(0) };
}
...
/// 参数：栈区间（起始地址，结束地址）
/// 返回：扫描到的根指针列表
pub type CustomStackScanner =
    unsafe fn(stack_start: *const u8, stack_end: *const u8) -> Vec<*mut u8>;

/// 自定义栈扫描器信息
struct CustomStackScannerInfo {
    scanner: CustomStackScanner,
...
pub struct Collector {
    ...
    /// 自定义栈扫描器列表
    custom_stack_scanners: Vec<CustomStackScannerInfo>,
}
...
impl Collector {
    ...
    unsafe fn correct_ptr(
        ...
        log::debug!(
            "gc {} [Cycle {}]: correct ptr result: {:?} in stack_loc={:p} from {:p} to {:p}",
            self.id,
            gc_cycle,
            re,
            father,
        ...
        unsafe {
            ...
            if self.thread_local_allocator().in_heap(ptr) {
                ...
                for i in 0..body_size / 8 {
                    let field_addr = (obj as *mut u8).add(i * 8 + 8);
        ...
        if self.thread_local_allocator().in_heap(ptr) && ptr as usize % BLOCK_SIZE > LINE_SIZE * 3 {
            ...
            if !is_candidate || obj_ref.byte_header.is_pinned() {
                ...
                if !new_ptr.is_null() {
                    ...
                    if is_eva_candidate {
                        log::error!(
                            "gc {} [Cycle {}]: BUG - evacuation target block is eva candidate! old={:p}, new={:p}",
                            self.id, gc_cycle, body, new_ptr
                        );
                    }
                    debug_assert!(!is_eva_candidate);
            ...
            match obj_ref.obj_type {
                ObjectType::Atomic => {
                    log::trace!("gc {}: skipping Atomic object at {:p}", self.id, body);
                }
    ...
    pub fn mark_fast_unwind(&self, sp: *mut u8) {
        // 递增全局GC计数器并设置当前线程的GC cycle
        let gc_cycle = GC_CYCLE_COUNT.fetch_add(1, Ordering::SeqCst) + 1;
        CURRENT_GC_CYCLE.with(|c| c.set(gc_cycle));
        log::info!("gc {}: === GC Cycle {} START ===", self.id, gc_cycle);
        ...
        #[cfg(feature = "llvm_stackmap")]
        {
            if USE_SHADOW_STACK.load(Ordering::Relaxed) {
                ...
                // 扫描自定义栈（例如 Karte 的虚拟栈）
                log::debug!("gc {} custom stack roots", self.id);
                self.mark_custom_stacks();
                log::debug!("gc {} custom stack roots end", self.id);
            }
        }
    ...
    unsafe fn mark_obj(&self, obj_type: ObjectType, obj: *mut u8) {
        ...
        match obj_type {
            ...
            ObjectType::Complex => {
                log::trace!("gc {}: mark_obj - Complex, calling mark_complex", self.id);
                self.mark_complex(obj);
            }
            ObjectType::Trait => {
                log::trace!("gc {}: mark_obj - Trait, calling mark_trait", self.id);
                self.mark_trait(obj);
            }
            ObjectType::Pointer => {
                log::trace!("gc {}: mark_obj - Pointer, calling mark_ptr", self.id);
                self.mark_ptr(obj);
            }
    ...
    pub fn collect_fast_unwind(&self, sp: *mut u8) {
        ...
        // 🔧 验证GC完成后所有根是否正确更新
        self.verify_custom_stack_roots();
    ...
    /// 注册自定义栈扫描器
    pub unsafe fn register_custom_stack_scanner(
        &mut self,
        scanner: CustomStackScanner,
        stack_start: *const u8,
        stack_end: *const u8,
    ) {
        self.custom_stack_scanners.push(CustomStackScannerInfo {
            scanner,
            stack_start,
            stack_end,
        });
    ...
    fn mark_custom_stacks(&self) {
        for scanner_info in &self.custom_stack_scanners {
            ...
            for root_ptr in roots {
                unsafe {
                    self.mark_ptr(root_ptr);
                }
            }
        }
    }

    /// 🔧 验证所有自定义栈根在GC后是否正确（无forward指针残留）
    fn verify_custom_stack_roots(&self) {
        let gc_cycle = CURRENT_GC_CYCLE.with(|c| c.get());
        ...
        for scanner_info in &self.custom_stack_scanners {
            ...
            for root_ptr in roots {
                unsafe {
                    // root_ptr 是栈位置，指向堆指针
                    let heap_ptr = *(root_ptr as *const *mut u8);

                    // 检查堆指针是否有效
                    if self.thread_local_allocator().in_heap(heap_ptr) {
                        if let Some(obj) = ImmixObject::from_unaligned_ptr(heap_ptr) {
                            let obj_ref = obj.as_ref().unwrap_unchecked();
                            let is_forwarded = obj_ref.byte_header.get_forwarded();

                            if is_forwarded {
                                log::error!(
                                    "gc {} [Cycle {}]: ⚠️  BUG - ROOT NOT UPDATED! stack_loc={:p} -> heap_ptr={:p} (STILL FORWARDED!)",
                                    self.id, gc_cycle, root_ptr, heap_ptr
                                );

                                // 读取forward指针
                                let body = obj_ref.get_body();
                                let forward_ptr = *(body as *const u64);
                                log::error!(
                                    "gc {} [Cycle {}]: ⚠️  Forward pointer: 0x{:X}, but stack still points to old address!",
                                    self.id, gc_cycle, forward_ptr
                                );
                            } else {
                                log::trace!(
