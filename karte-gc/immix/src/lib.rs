...
/// 注册自定义栈扫描器到当前线程的 Collector
/// 这允许用户提供自定义的栈扫描逻辑，例如扫描虚拟栈
pub fn register_custom_stack_scanner(
    scanner: collector::CustomStackScanner,
    stack_start: *const u8,
    stack_end: *const u8,
) {
    SPACE.with(|gc| unsafe {
        if let Some(gc) = gc.get().as_mut() {
            gc.register_custom_stack_scanner(scanner, stack_start, stack_end);
        }
    });
}
