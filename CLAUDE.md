...
- **INCREMENT_BLOCK MIR BLOCK ORDER GOTCHA**: ForIn 和 ForArray 的 `increment_block` 在 MIR 中按创建顺序（HashMap key 排序）排在 loop body blocks 之前。LIR 按 MIR block 顺序生成代码。**绝不能在 `increment_block` 中添加引用 loop body 变量的语句**（HeapAlloc/Store/Dereference），因为第一次迭代时这些变量未初始化。正确做法：在 `normal_end_block`（loop body 的最后一个 block）中使用 `wrapped_normal_values` 预创建 shared_var，然后 `actual_backedge_values` 直接复用。相关修复见 `karte-mir/src/lower/expr.rs` 搜索 "关键修复：不在 increment_block 中"。
