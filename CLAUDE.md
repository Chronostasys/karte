...
- **AMD SPIR-V OPTRACCESSCHAIN IMM OFFSET GOTCHA**: `addr_pattern_map` 的 fallback 代码把 `Add(Imm(字节偏移), Param(指针))` 中的 `Imm` 直接作为元素索引传给 `OpPtrAccessChain`。但 `OpPtrAccessChain(ptr, index)` 的 index 是**元素数量**（自动乘 sizeof），不是字节偏移。例如 `Imm(4)` 被解释为 ptr[4]（第5个 f32）而非 ptr[1]（第2个 f32，偏移4字节）。**修复**: 在 GlobalLoad/GlobalStore 中，当 `offset_op` 是 `Imm` 时，除以 `dtype.size_in_bytes()` 转换为元素索引。`Mul(tid, sizeof) + Param` 模式不受影响（提取的是 tid Reg，已是元素索引）。
...
- **🔴 任何时候回复用户禁止说"遗留问题"、"后续修复"、"已知限制"、"out of scope"之类的话。回复用户必须没有任何未完成事项。必须全部修完才能回复。** 🔴
- **🔴 任何时候回复用户禁止说"遗留问题"、"后续修复"、"已知限制"、"out of scope"之类的话。回复用户必须没有任何未完成事项。必须全部修完才能回复。** 🔴
- **🔴 任何时候回复用户禁止说"遗留问题"、"后续修复"、"已知限制"、"out of scope"之类的话。回复用户必须没有任何未完成事项。必须全部修完才能回复。** 🔴
- **🔴 任何时候回复用户禁止说"遗留问题"、"后续修复"、"已知限制"、"out of scope"之类的话。回复用户必须没有任何未完成事项。必须全部修完才能回复。** 🔴
- **🔴 任何时候回复用户禁止说"遗留问题"、"后续修复"、"已知限制"、"out of scope"之类的话。回复用户必须没有任何未完成事项。必须全部修完才能回复。** 🔴
