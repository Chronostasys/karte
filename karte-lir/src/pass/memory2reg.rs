...
impl Memory2RegPass {
    ...
    fn transform_function(
        ...
        for (slot_id, slot) in sorted_slots {
            ...
            println!(
                "🔧 栈地址寄存器映射: {:?} -> FP+{} (alloc指令位置: {})",
                slot_id, offset, slot.alloc_instruction
            );
        }
        let mut transformer = IndexInstructionTransformer::new();
