...
impl Default for SsaConstructionPass {
    fn default() -> Self {
        Self::new()
    }
}
...
impl SsaConstructionPass {
    ...
    fn build_control_flow_graph(&mut self, function: &LirFunction) -> Result<(), String> {
        ...
        // 收集并排序基本块开始位置
        let mut block_starts: Vec<(Option<crate::LabelId>, usize)> =
            leaders.into_iter().collect::<Vec<(Option<_>, usize)>>();
                ...
                match last_instruction {
                    Instruction::Jump { target, .. } => {
                        ...
                        if let Some(target_block) = self.find_block_by_label(*target) {
                            self.control_flow_graph[block_idx]
                                .successors
                                .push(target_block);
                            self.control_flow_graph[target_block]
                                .predecessors
                                .push(block_idx);
                        }
                    }
                    ...
                    | Instruction::JumpNotEqual { target, .. } => {
                        ...
                        if block_idx + 1 < self.control_flow_graph.len() {
                            ...
                            self.control_flow_graph[block_idx]
                                .successors
                                .push(next_block);
                            self.control_flow_graph[next_block]
                                .predecessors
                                .push(block_idx);
                        }
                    }
    ...
    ) -> Result<HashMap<Register, PhiNode>, String> {
    ...
    fn needs_phi_node(&self, _register: Register, block_id: usize) -> bool {
        ...
        self.control_flow_graph
            .get(block_id)
            .is_some_and(|block| block.predecessors.len() > 1)
    }
            ...
            if i < function.instructions.len() {
                ...
                if let Some(def_reg) = self.get_defined_register(&function.instructions[i]) {
                    ...
                    *version += 1;
                    ...
                    version_stacks.entry(def_reg).or_default().push(*version);
