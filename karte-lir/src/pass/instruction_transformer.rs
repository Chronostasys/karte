...
impl IndexInstructionTransformer {
    ...
    pub fn replace(&mut self, index: usize, new_instruction: Instruction) {
        self.transforms.push(IndexTransformOperation::Replace(index, new_instruction));
    }
    
    /// 添加插入操作
    pub fn insert(&mut self, index: usize, new_instruction: Instruction) {
        self.transforms.push(IndexTransformOperation::Insert(index, new_instruction));
    }
...
impl HistoryBasedTransformer {
    ...
    pub fn apply_to_function(&mut self, function: &mut LirFunction) -> (bool, usize, usize, usize) {
        ...
        println!("🧠 智能变换系统开始：{} 个操作，{} 条历史记录", 
                self.operations.len(), self.transform_history.len());
        ...
        println!("✅ 智能变换系统完成，历史记录数量：{}", self.transform_history.len());
        (modified, removed_count, replaced_count, inserted_count)
    }
            ...
            if record.target_position < original_index {
                match record.transform_type {
                    TransformType::Remove => {
                        ...
                        println!("🧠     删除影响：序号 {} -> {}", corrected_index + 1, corrected_index);
                    }
                    TransformType::Insert => {
                        corrected_index += 1;
                        println!("🧠     插入影响：序号 {} -> {}", corrected_index - 1, corrected_index);
                    }
    ...
    /// 应用单个操作
    fn apply_single_operation(&mut self, function: &mut LirFunction, operation: &HistoryBasedOperation, corrected_index: usize) -> bool {
        println!("🧠 应用单个操作：序号 {}, 类型 {:?}", corrected_index, operation.operation_type);
        ...
        match &operation.operation_type {
            HistoryBasedOperationType::Remove => {
                println!("🧠   删除指令 [{}]: {:?}", corrected_index, function.instructions[corrected_index]);
            ...
            HistoryBasedOperationType::Replace(new_instruction) => {
                println!("🧠   替换指令 [{}]: {:?} -> {:?}", corrected_index, function.instructions[corrected_index], new_instruction);
            ...
            HistoryBasedOperationType::Insert(new_instruction) => {
                ...
                function.instructions.insert(corrected_index, new_instruction.clone());
            }
        }
...
impl BatchTransformer {
    ...
    pub fn apply_to_function(&mut self, function: &mut LirFunction) -> BatchTransformResult {
        ...
        // 先应用 index-based 变换
        let (index_changed, index_removed_count, index_replaced_count, index_inserted_count) = self.index_transformer.apply_to_function(function);
        if index_changed {
            result.changed = true;
        ...
        let (history_changed, history_removed_count, history_replaced_count, history_inserted_count) = self.history_transformer.apply_to_function(function);
        if history_changed {
            result.changed = true;
...
pub mod utils {
    ...
    /// 安全地替换指令
    pub fn safe_replace_instruction(function: &mut LirFunction, index: usize, new_instruction: Instruction) -> bool {
        if index < function.instructions.len() {
    ...
    /// 安全地插入指令
    pub fn safe_insert_instruction(function: &mut LirFunction, index: usize, new_instruction: Instruction) -> bool {
        if index <= function.instructions.len() {
    ...
    /// 批量替换指令
    pub fn batch_replace_instructions(function: &mut LirFunction, replacements: &[(usize, Instruction)]) -> usize {
        let mut replaced_count = 0;
    ...
    /// 批量插入指令（从后往前，避免索引问题）
    pub fn batch_insert_instructions(function: &mut LirFunction, insertions: &[(usize, Instruction)]) -> usize {
        let mut sorted_insertions: Vec<(usize, Instruction)> = insertions.iter().cloned().collect();
...
mod tests {
    ...
    fn test_index_based_transformer() {
        ...
        // 应用变换
        let (changed, removed_count, replaced_count, inserted_count) = transformer.apply_to_function(&mut function);
