...
use karte_lir::{LirProgram, LirFunction, Instruction, LabelId};
...
impl ProgramManager {
    ...
    fn setup_main_function(&mut self, program: &LirProgram) -> Result<(), String> {
        let main_name = program.main_function.as_ref()
            .ok_or("No main function specified")?;
            
        let func_info = self.function_map.get(main_name)
            .ok_or("Main function not found in function map")?;
            
        let entry_label = func_info.entry_label
            .ok_or("Main function has no entry label")?;
            
        let entry_pc = *self.label_map.get(&entry_label)
    ...
    pub fn get_function_at_pc(&self, pc: usize) -> Option<&FunctionInfo> {
        for func_info in self.function_map.values() {
            if pc >= func_info.start_pc && pc < func_info.end_pc {
