...
impl JitManager {
    ...
    pub fn compile_function(
        ...
        if let Some(cached) = self.code_cache.get(&function.name) {
            if self.debug_mode {
                println!("JIT: 从缓存获取函数 '{}'", function.name);
            }
            return Ok(cached.clone());
        }

        if self.debug_mode {
            println!("JIT: 编译函数 '{}'", function.name);
        }
        ...
        // 缓存结果
        self.code_cache.insert(function.name.clone(), compiled.clone());
    ...
    /// 编译程序
    pub fn compile_program(&mut self, program: &LirProgram) -> Result<CompiledProgram, String> {
        let mut compiled_functions = HashMap::new();

        // 编译所有函数
        for (name, function) in &program.functions {
            let compiled = self.compile_function(function, program)?;
            compiled_functions.insert(name.clone(), compiled);
        }

        let main_function = program.main_function.clone();
...
mod tests {
    ...
    fn create_test_function() -> LirFunction {
        ...
        function.instructions.push(Instruction::Add {
            ...
            src2: Operand::Register { id: Register::Virtual(2) },
            span: Span::dummy(),
        });
    ...
    fn test_program_compilation() {
        ...
        program.set_main("test_add".to_string());
        
        let result = jit.compile_program(&program);
        assert!(result.is_ok());
        
        let compiled_program = result.unwrap();
        assert_eq!(compiled_program.main_function, Some("test_add".to_string()));
        assert!(compiled_program.functions.contains_key("test_add"));
    }
} 