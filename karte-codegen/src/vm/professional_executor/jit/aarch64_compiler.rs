...
impl AArch64Compiler {
    ...
    fn emit_lsl_reg(&self, code_builder: &mut CodeBuilder, dst: u8, src1: u8, src2: u8) {
        // LSL Xd, Xn, Xm = UBFM Xd, Xn, (-Xm mod 64), (63-Xm)
        // 等价于: 31|00|11010110|Xm|001011|Xn|Xd
        let instruction =
            0x9AC02C00u32 | ((src2 as u32) << 16) | ((src1 as u32) << 5) | (dst as u32);
        code_builder.emit_bytes(&instruction.to_le_bytes());
    }
    ...
    fn emit_asr_reg(&self, code_builder: &mut CodeBuilder, dst: u8, src1: u8, src2: u8) {
        // ASR Xd, Xn, Xm = SBFM Xd, Xn, Xm, 63
        // 编码: sf=1, opc=00, 100110, N=1, immr=Xm, imms=011111, Rn, Rd
        let instruction =
