...
impl X86Runtime {
    ...
    /// __karte_string_substring(str_ptr, start, length) → 新字符串指针
    ///
    /// RDI(7) = str_ptr, RSI(6) = start, RDX(2) = length
    /// 返回 RAX = 新字符串指针
    ///
    /// 字符串布局: [length: i64][bytes...]
    /// 算法:
    ///   1. 空指针检查 → 返回 0
    ///   2. 边界检查: start >= src_len → 返回空字符串
    ///   3. actual_length = min(length, src_len - start)
    ///   4. GC 安全：先复制源数据到系统栈缓冲区，再 gc_alloc
    ///   5. 分配新字符串，复制数据，清零尾部对齐字节
    fn emit_string_substring(&mut self) {
        self.fn_start(runtime_names::STRING_SUBSTRING);
        ...
        // RBX = src_len
        self.mov_mem_load(3, 7, 0); // RBX = [RDI+0]
        // R12 = str_ptr（保存，rep movsb 会破坏 RDI/RSI）
        self.mov_rr(12, 7); // R12 = RDI (str_ptr)

        // ---- 边界检查: start >= src_len → 空字符串 ----
        self.cmp_rr(6, 3); // CMP RSI(start), RBX(src_len)
        self.jae_rel32(0); // 如果 start >= src_len → empty_string
        let start_oob_patch = self.code.len() - 4;

        // RBP = actual_start = start = RSI
        self.mov_rr(5, 6); // RBP = RSI (start)

        // RBX = available = src_len - start
        self.sub_rr(3, 6); // RBX -= RSI → available

        // actual_length = min(length, available)
        // RBX = available, RDX = length
        // 如果 length > available → JA 跳到 length_ok（RBX 已经是 available）
        // 否则 → RBX = RDX (length)
        self.cmp_rr(2, 3); // CMP RDX(length), RBX(available)
        // JA rel32: 0x0F 0x87 + rel32
        self.bs(&[0x0F, 0x87]);
        self.u32(0); // 占位 rel32
        let length_ok_patch = self.code.len() - 4;

        // length <= available: RBX = RDX (length)
        self.mov_rr(3, 2); // RBX = RDX (length)

        // .length_ok:
        let length_ok_label = self.code.len();

        // ---- 在系统栈上分配缓冲区 ----
        // aligned_size = (actual_length + 7) & ~7
        self.mov_rr(0, 3);  // RAX = RBX (actual_length)
        self.add_ri8(0, 7); // RAX += 7
        self.and_ri32(0, 0xFFFFFFF8); // RAX &= ~7 (对齐到 8)
        // SUB RSP, RAX
        self.sub_rr(4, 0); // RSP -= RAX
        ...
        self.call_rel32(0); // 占位 CALL，稍后修补到 gc_alloc
        self.functions.push(RuntimeFunction {
            name: "__string_substring_call_alloc".to_string(),
            offset: call_pos,
            size: 5,
        });
        ...
        // ---- 写 actual_length 到 [RAX + 0] ----
        self.mov_mem_store(0, 0, 3); // [RAX+0] = RBX (actual_length)

        // ---- 从栈缓冲区复制到 [RAX + 8] ----
        // ⚠️ 必须先设置 RSI = RSP（栈缓冲区），再 push RAX
        // 否则 push 会修改 RSP，导致 RSI 指向错误位置
        self.mov_rr(6, 4);  // RSI = RSP (栈缓冲区)
        self.push(0); // 保存 RAX (result ptr)
        self.mov_rr(7, 0);  // RDI = RAX
        self.add_ri8(7, 8); // RDI = RAX + 8 (数据区)
        self.mov_rr(1, 3);  // RCX = RBX (actual_length)
        self.bs(&[0xF3, 0xA4]); // REP MOVSB

        // ---- 清零尾部对齐字节 ----
        self.pop(0); // 恢复 RAX
        // remaining = actual_length & 7
        self.mov_rr(1, 3); // RCX = actual_length
        ...
        // padding_start = RAX + 8 + actual_length
        self.push(0); // 保存 RAX
        self.add_ri8(0, 8); // RAX += 8
        self.add_rr(0, 3);  // RAX += actual_length → padding_start
        // 清零循环：R11 = 0, 逐字节写
        ...
        self.add_ri8(0, 1); // RAX++
        self.sub_ri8(2, 1); // RDX--
        self.jmp_rel32(0);
        ...
        // ---- empty_string: start >= src_len，分配空字符串 ----
        let empty_string_label = self.code.len();
        ...
        self.test_rr(0, 0);
        self.jz_rel32(0);
        let empty_oom_patch = self.code.len() - 4;
        // 写入 length = 0
        self.push(0);
        ...
        // ---- empty_oom: empty_string 中 gc_alloc 返回 0 ----
        let empty_oom_label = self.code.len();
        self.mov_rr(4, 13); // RSP = R13
        self.xor_rr(0, 0);
        self.pop(13);
        self.pop(12);
        ...
        let rel = null_label as i32 - (null_patch as i32 + 4);
        self.code[null_patch..null_patch + 4].copy_from_slice(&rel.to_le_bytes());
        let rel = empty_string_label as i32 - (start_oob_patch as i32 + 4);
        self.code[start_oob_patch..start_oob_patch + 4].copy_from_slice(&rel.to_le_bytes());
        let rel = length_ok_label as i32 - (length_ok_patch as i32 + 4);
        self.code[length_ok_patch..length_ok_patch + 4].copy_from_slice(&rel.to_le_bytes());
        let rel = oom_label as i32 - (oom_patch as i32 + 4);
        self.code[oom_patch..oom_patch + 4].copy_from_slice(&rel.to_le_bytes());
        ...
        let rel = empty_oom_label as i32 - (empty_oom_patch as i32 + 4);
        self.code[empty_oom_patch..empty_oom_patch + 4].copy_from_slice(&rel.to_le_bytes());
    }

    /// __karte_string_contains(str_ptr, char_code) → 0 或 1
    ///
    /// RDI(7) = str_ptr, RSI(6) = char_code
    /// 返回 RAX = 1（找到）或 0（未找到）
    ///
    /// 不触发 GC（无内存分配，纯逐字节扫描）
    fn emit_string_contains(&mut self) {
        self.fn_start(runtime_names::STRING_CONTAINS);
        ...
        // ---- 读 len = [str_ptr + 0] ----
        self.mov_mem_load(5, 7, 0); // RBP = [RDI+0]

        // ---- 如果 len == 0 → 返回 0 ----
        self.test_rr(5, 5); // TEST RBP, RBP
        self.jz_rel32(0);
        ...
        // CMP index, len
        self.cmp_rr(3, 5); // CMP RBX, RBP
        self.jae_rel32(0); // 如果 index >= len → not_found
        let not_found_patch = self.code.len() - 4;

        // 加载 byte [str_ptr + 8 + index]
        self.mov_rr(0, 7);  // RAX = RDI (str_ptr)
        self.add_ri8(0, 8); // RAX += 8
        self.add_rr(0, 3);  // RAX += RBX (index)
        self.movzx_byte(2, 0, 0); // MOVZX RDX, byte [RAX]

        // CMP byte_val, char_code
        self.cmp_rr(2, 6); // CMP RDX, RSI
        self.je_rel32(0);  // 如果相等 → found
        let found_patch = self.code.len() - 4;
        ...
        // JMP .loop
        self.jmp_rel32(0);
        let loop_jmp_patch = self.code.len() - 4;
        let rel = loop_start as i32 - (loop_jmp_patch as i32 + 4);
        self.code[loop_jmp_patch..loop_jmp_patch + 4].copy_from_slice(&rel.to_le_bytes());

        // .found:
        let found_label = self.code.len();
        self.mov_ri(0, 1); // RAX = 1
        self.pop(3);  // RBX
        self.pop(5);  // RBP
        self.ret();

        // .not_found:
        let not_found_label = self.code.len();
        self.xor_rr(0, 0); // RAX = 0

        // .done (falls through from not_found):
        self.pop(3);  // RBX
        self.pop(5);  // RBP
        self.ret();
        ...
        // ---- 修补所有跳转 ----
        let rel = not_found_label as i32 - (not_found_patch as i32 + 4);
        self.code[not_found_patch..not_found_patch + 4].copy_from_slice(&rel.to_le_bytes());
        let rel = found_label as i32 - (found_patch as i32 + 4);
        self.code[found_patch..found_patch + 4].copy_from_slice(&rel.to_le_bytes());
        let rel = null_label as i32 - (null_patch as i32 + 4);
        self.code[null_patch..null_patch + 4].copy_from_slice(&rel.to_le_bytes());
    ...
    /// __karte_split_count(str_ptr, delimiter) → 分割段数
    ///
    /// RDI(7) = str_ptr, RSI(6) = delimiter
    /// 返回 RAX = 分割后的段数（分隔符出现次数 + 1）
    ///
    /// 不触发 GC（无内存分配，纯扫描计数）
    fn emit_split_count(&mut self) {
        self.fn_start(runtime_names::SPLIT_COUNT);
        ...
        // RBX = len = [str_ptr + 0]
        self.mov_mem_load(3, 12, 0); // RBX = [R12+0]
        ...
        self.mov_ri(5, 1); // RBP = 1
        // RCX(1) = index = 0
        self.xor_rr(1, 1); // XOR RCX, RCX

        // .loop:
        let loop_start = self.code.len();

        // CMP index, len
        self.cmp_rr(1, 3); // CMP RCX, RBX
        self.jae_rel32(0); // 如果 index >= len → done
        let done_patch = self.code.len() - 4;

        // 加载 byte [str_ptr + 8 + index]
        self.mov_rr(0, 12); // RAX = R12 (str_ptr)
        self.add_ri8(0, 8); // RAX += 8
        self.add_rr(0, 1);  // RAX += RCX (index)
        self.movzx_byte(2, 0, 0); // MOVZX RDX, byte [RAX]

        // CMP byte_val, delimiter
        self.cmp_rr(2, 6); // CMP RDX, RSI
        self.jne_rel32(0); // 如果不相等 → next
        let next_patch = self.code.len() - 4;

        // count++
        self.add_ri8(5, 1); // RBP += 1

        // .next:
        let next_label = self.code.len();

        // index++
        self.add_ri8(1, 1); // RCX += 1
        ...
        let rel = next_label as i32 - (next_patch as i32 + 4);
        self.code[next_patch..next_patch + 4].copy_from_slice(&rel.to_le_bytes());
        let rel = null_label as i32 - (null_patch as i32 + 4);
        self.code[null_patch..null_patch + 4].copy_from_slice(&rel.to_le_bytes());
        let rel = zero_len_label as i32 - (zero_len_patch as i32 + 4);
        self.code[zero_len_patch..zero_len_patch + 4].copy_from_slice(&rel.to_le_bytes());
